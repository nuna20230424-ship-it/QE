...
- 뷰: **현황 보드**(상태별 칸반) · **일정표**(일정순 테이블) · **캘린더**(월간) · **일일보고 / 주간보고** · **인증 통계**(모델별) · 등록/상세 모달
...
| 영역 | 항목 |
|------|------|
| 의뢰 정보 (개발 PL) | 인증종류(Netflix NTS / Google xTS / Amazon AVTS), Test type(IR/LR/MR/파생), Test 목적(3PL/Official/Pre-Test/양산/self/MR), Round, 모델명(드롭다운+직접입력), FW 버전, 의뢰자(드롭다운+직접입력), 희망일정, 비고 |
...
## 인증 통계 (모델별)
**인증 통계** 탭에서 프로젝트(모델) × 인증종류별로 다음을 집계한다. 상단에서 **주간(월~금)**과 **전체 누적**을 전환하고, 주간 모드에서는 `‹ 8/24~8/28 ›`로 주를 넘긴다(대상 기간은 `2026-08-24 ~ 2026-08-28 (월~금)`로 함께 표기). **⤓ 엑셀 다운로드**는 화면에 보이는 집계를 그대로 CSV(UTF-8 BOM)로 내려받는다 — 파일명 `인증통계_2026-08-24_2026-08-28.csv`, 첫 줄에 대상 기간, 마지막 줄에 합계.

| 지표 | 정의 |
|------|------|
| 진행차수 | 해당 모델·인증의 의뢰 횟수 (주간 모드 = 그 주, 누적 모드 = 전체) |
| 최근 Round 입력 | 의뢰에 수동 입력된 `Round` 값의 최댓값 |
| Pass / Fail / 미판정 | 판정별 건수 |
| Pass율 / Fail율 | **전체 의뢰 횟수** 대비 비율 |

> 비율의 분모는 전체 의뢰 횟수(미판정 포함)이므로 **Pass율 + Fail율이 100%가 되지 않는다.** 의뢰가 0건이면 비율은 0%로 반환한다(0 나눗셈 방어).
