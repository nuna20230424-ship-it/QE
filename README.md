...
- **1주 가용을 100%로 산정한다.** `인증 담당 4명 × 5 영업일 = 20 slot`이 100%이고, 미완 물량이 그 몇 %인지로 사용률·여유·초과를 낸다.
- 영업일은 **주말·공휴일을 제외**한다.
...
### 상단 — 1주 가용 기준 정량 현황 (담당자 이름 없음)
| 지표 | 정의 |
|------|------|
| 1주 가용 (100%) | `인원수 × 5 영업일` = **20 slot** |
| 미완 총 소요 | 대상 의뢰의 slot 합계 |
| 사용률 | `미완 ÷ 1주 가용` (%). 100%를 넘으면 1주에 못 끝난다 |
| 여유 / 초과 | 100% 미만이면 `여유 N% (N slot 배정 가능)`, 넘으면 `초과 N% (N slot 넘침)` |
| 소요 주수 | `미완 ÷ 1주 가용` (주). `2.4주`처럼 읽는다 |
| 미배정 | 담당 테스터가 비어 있는 물량 (1주 가용 대비 % 병기) |
| 전체 소진 예상 / 완료일 | 4명이 하루 4 slot을 소화할 때 필요한 영업일수와 그 날짜 |

### 일별 가용 리소스 현황
향후 20 영업일(기본, `?days=`로 5~60)에 대해 **날짜별 가용 / 배정 / 배정예정 / 여유 / 사용률**을 보여주고, 주 단위 소계를 얹는다. 공휴일이 든 주는 영업일이 줄어 **가용도 함께 줄어든다** (추석 주 → 영업일 3일 × 4명 = 12 slot).

배치 규칙은 이렇다.

- **한 담당자는 하루 1 slot만 쓴다**(직렬). 그래서 의뢰를 담당자별로 줄 세워 영업일에 이어 붙인다. 시작은 `max(대표 일정, 기준일)`이다.
- **`배정예정`** 은 담당 테스터가 없는 건을 **가장 빨리 비는 담당자 자리에 채워 본 자동 할당 미리보기**다. 실제 배정이 아니라 예측이므로 색을 달리 표시한다.
- 각 날짜에 **그날 비어 있는 담당자**를 흐린 칩으로 함께 보여준다 — 자동 할당이 어디에 넣을 수 있는지가 바로 보인다.
- **4명 외 테스터의 물량은 4명의 가용을 쓰지 않으므로 배치에서 제외**하되, 몇 slot이 왜 빠졌는지 화면에 명시한다. 조회 구간을 넘어간 물량도 `overflow`로 표기한다. **배치분 + 구간초과 + 제외분 = 전체 물량**이 항상 성립하도록 테스트로 고정했다.
...
- **기타**는 4명 외 테스터(김지윤·조진원 등). 전체 물량이 어긋나지 않게 함께 표시한다. 미배정·기타는 개인 가용 개념이 없어 사용률을 산출하지 않는다(`—`).
- 담당자 행 아래 `건별 상세`를 펼치면 그 slot이 어느 의뢰에서 왔는지(일정·인증종류·모델명·차수·적용 규칙)를 볼 수 있다.

조회는 `GET /api/resources` (`?as_of=YYYY-MM-DD` 기준일, `?days=N` 일별 구간).
