...
| `요청 실패: Confluence 응답 401 ...` | 인증 거부 | **스크립트가 자동으로 토큰 점검을 이어서 한다.** 아래 참조 |
...
## 7-B. 401 이 나왔을 때 — 스크립트가 원인을 갈라 준다

401·403 이면 스크립트가 캘린더와 무관한 엔드포인트(`/rest/api/user/current`)를 한 번 더 불러
**토큰 문제인지 캘린더 문제인지** 갈라 준다.

```
토큰 자체가 먹는지 확인한다 (캘린더와 무관한 엔드포인트)
  GET /rest/api/user/current → 200
  토큰은 정상이다 (계정: k251110).
  → 토큰 문제가 아니다. 그 캘린더 열람 권한이나 subCalendarId 를 확인한다.
```

```
토큰 자체가 먹는지 확인한다 (캘린더와 무관한 엔드포인트)
  GET /rest/api/user/current → 401
  여기서도 401 다 → 토큰이 이 인스턴스에서 먹지 않는다.
  → 토큰을 다시 발급하거나(만료·복사 누락), PAT 가 켜져 있는지 관리자에게 확인한다.
```

오류 메시지에는 **응답 본문 앞부분**도 함께 실린다(`· 본문: ...`). 401의 실제 이유가
대개 여기 적혀 있다 — `Personal access tokens are disabled`, `Basic authentication ... not allowed` 같은 문장.
