...
## 자동 보고 메일
서버가 아래 일정으로 보고서를 메일 발송합니다. 본문은 화면에서 보는 것과 **같은 템플릿·서식**이고, 각 메일에 **엑셀(CSV) 첨부**가 붙습니다.

| 보고 | 발송 시각 | 집계 대상 | 첨부 |
|------|-----------|-----------|------|
| 일일보고 | **매일 18:00** | 오늘 하루 | `일일현황보고_YYYY-MM-DD.csv` |
| 주간보고 | **매주 금요일 18:00** | 이번 주 월~일 (인증통계 섹션만 월~금) | `주간현황보고_*.csv` + `인증통계_*.csv` |
| 인증 통계 | **매주 월요일 09:00** | **지난주** 월~금 | `인증통계_*.csv` |

> 월요일 아침에는 이번 주 실적이 아직 비어 있으므로 인증 통계 보고는 **지난주 월~금**을 싣습니다.

- 수신: `config.reportTo` (미설정 시 기본값 `nuna20230424@gmail.com`, `keonhee.cho@kaongroup.com`)
- 첨부는 화면의 **⤓ 엑셀 다운로드**와 같은 UTF-8 BOM CSV라 Excel에서 바로 열립니다.
