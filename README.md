...
| 표기 | 정의 |
|------|------|
| 할당 slot / 소요 영업일 | 그 담당자에게 쌓인 slot 합계 (`1 slot = 1일`) |
| 예상 소진일 | 오늘부터 세어 **할당 slot번째 영업일** |
| 기준선 대비 | **4명 균등분담 기준선**(총 slot ÷ 4) 대비 여유(+) / 초과(−) |
| 부하 편차 | 4명 중 최다 − 최소. 재배정 필요 여부 판단용 |
| 미배정 | 담당 테스터가 비어 있는 건. 별도 행으로 분리하되 **전체 합계에 포함**한다 |
| 기타 | 4명 외 테스터(김지윤·조진원 등)에게 걸린 건. 전체 물량이 어긋나지 않게 함께 표시한다 |

담당자 행 아래 `건별 상세`를 펼치면 그 slot이 어느 의뢰에서 왔는지(일정·인증종류·모델명·차수·적용 규칙)를 확인할 수 있다. 조회는 `GET /api/resources` (`?as_of=YYYY-MM-DD`로 기준일 지정 가능).
